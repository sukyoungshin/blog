...
const CustomDropdownMenu = () => {
  ...
  return (
    <CustomSelectMenuWrapper>
      <SelectMenuWrapper>
        <button type="button" onClick={openDropdownMenu} className="initial-select">
          {SelectMenuLists[selectedId]?.text}
        </button>
        <SelectMenuButton openDropdownMenu={openDropdownMenu} />
      </SelectMenuWrapper>
      {visible && (
        <SelectMenuToast
...
const SelectMenuButton = ({ ...props }: any) => {
  return (
    <Button size={32} active={false} onClick={props.openDropdownMenu}>
